...
export class DiscordBotService {
  ...
  constructor(private readonly env: BotEnv, commands: SlashCommand[]) {
...
export const createCoreSlashCommands = (services: BotCommandServices): SlashCommand[] => {
  ...
  const setupTimezone = new SlashCommandBuilder()
    ...
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addStringOption((option) =>
      option.setName('timezone').setDescription('IANA timezone, e.g. America/Toronto').setRequired(true)
    );
  ...
  const commands: SlashCommand[] = [
    ...
    },
    {
      ...
      execute: async (interaction) => {
        ...
        if (
          linkRow.verificationExpiresAt &&
          linkRow.verificationExpiresAt.getTime() < Date.now()
        ) {
          await interaction.reply({
...
const buildWeeklyLeaderboardEmbed = (
  snapshot: WeeklyLeaderboardSnapshotPayload
): EmbedBuilder => {
